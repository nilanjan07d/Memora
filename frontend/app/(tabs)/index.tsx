...
export default function HomeScreen() {
  ...
  async function loadData() {
    try {
      ...
      const validLatestJourneys =
        Array.isArray(latestJourneys)
          ? latestJourneys.filter(Boolean)
          : [];

      if (validLatestJourneys.length > 0) {
        const latestJourney =
          validLatestJourneys[0];

        if (latestJourney?._id) {
          await fetchJourneyMemories(
            latestJourney._id
          );
        }
      }
      ...
      <View style={styles.section}>
        ...
        {visibleJourneys.length === 0 ? (
          ...
          visibleJourneys
            ...
            .map((journey) => {
              /*
               * Extra protection in case a malformed
               * object somehow gets into the array.
               */
              if (!journey) {
                return null;
              }
